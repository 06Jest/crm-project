...
export default function ChatPanel() {
  ...
  const { items: deals, loaded: dLd } = useSelector(
    (s: RootState) => s.deals
  );

  const { items: customers, loaded: cuLd } = useSelector(
    (s: RootState) => s.customers
  );

  const { items: members, loaded: pLd } = useSelector(
    (s: RootState) => s.orgmembers
  );
  ...
  }, [userId, memberId, realtimeReady, dispatch]);
  ...
  const conversationsRefreshTimeout = useRef<
  ...
  const getDisplayName = (
    ...
    return conversation.other_participant
      ? `${conversation.other_participant.profile.first_name} ${conversation.other_participant.profile.last_name}`
      : "DM";
  };
  ...
  const getValue = (
    ...
    switch (type) {
      ...
      case "deal": {
        ...
        return contact
          ? `${formatName(contact.first_name, contact.last_name)} ${formatShortTitle(deal.title)}`
          : "";
      }
  ...
  const entityOptions: EntitySelectOption[] = useMemo(() => {
    switch (entityType) {
      ...
      case "deal":
        return deals.map((d) => ({
          id: d.id,
          label: formatTitle(d.title),
        }));

      case "customer":
        return customers.map((c) => {
          const contact = contacts.find(
            (co) => co.id === c.contact_id
          );
  ...
  const handleSend = async () => {
    ...
    const payload: AddMessage = {
      ...
      entity_type:
        entityType === "none" ? null : entityType,
      entity_id:
        entityType === "none" ? null : entityId || null,
    };
      ...
      dispatch(
        updateRealtimeConversation({
          ...
          last_message: {
            ...
            sender: {
              id: sent.sender_id ?? sent.sender.id,
            },
          },
  ...
  const canModifyMessage = (
    ...
    const age =
      now - new Date(message.created_at).getTime();

    return (
      age < MESSAGE_MODIFY_WINDOW_MS &&
  ...
  const renderConversationRow = (
    ...
    let avatar = (
      <Avatar
        sx={{
          bgcolor:
            stringToAvatarColor(displayName),
        }}
      >
    ...
    if (conversation.type === "announcement") {
      ...
      avatar = (
        <Avatar
          src={
            conversation.other_participant.profile
    ...
    >
      <Box
        sx={{
          ...
          minHeight: 0,
          overflow: "hidden",
          "@container chat-panel (min-width: 700px)": {
            flexDirection: "row",
          },
        }}
      >
          ...
          {convLoading && !convLoaded ? (
            ...
            >
              <List
                ...
                disablePadding
              >
                {grouped.announcements.map(
                  renderConversationRow
                )}

                {grouped.organization.map(
                  renderConversationRow
                )}
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    by sending your first
                    message. You can also
                    attach a related contact,
                                  ...
                                  message.entity_id && (
                                    <Paper
                                      onClick={(e) => {
                                        ...
                                        if (message.entity_type ==="deal") {
                                          return;
                                        }
                                        e.stopPropagation();

                                        navigate(
                                          `/app/${message.entity_type}s/${message.entity_id}`
                                        );
                                      }}
                                      title={`View ${getValue(
                                        message.entity_type,
